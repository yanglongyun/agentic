...
export async function execute(code, config, signal) {
  ...
  if (!process.connected) {
    throw new Error("Mac 控制工具需要在 agentic 桌面 App 中使用");
  }
  if (running) {
    throw new Error("另一个Mac 控制脚本正在执行，请等它结束后重试");
  }
  ...
  const timer = setTimeout(
    () => controller.abort(new Error("Mac 控制脚本执行超过 60 秒；请读取页面确认操作结果")),
    60000,
  );
  try {
    return await new Promise((resolve, reject) => {
      ...
      async function receive(message) {
        if (message.type === "computer:stopped" && message.runId === runId) {
          controller.abort(new Error("Mac 控制已被用户停止，请等待用户重新开启"));
          return;
        }
        if (message.type !== "computer:result" || message.runId !== runId || closed) {
          return;
        }
        try {
      ...
      worker.once("exit", (code) => {
        ...
        if (!closed) {
          reject(new Error(`Mac 控制脚本线程已退出（${code}）`));
        }
      });
      ...
      worker.on("message", (message) => {
        ...
        if (message.type === "request") {
          process.send({ ...message, type: "computer:request", runId }, (error) => {
            if (error) {
              reject(error);
            }
          });
    ...
    clearTimeout(timer);
    if (process.connected) {
      process.send({ type: completed ? "computer:end" : "computer:cancel", runId }, () => {});
    }
